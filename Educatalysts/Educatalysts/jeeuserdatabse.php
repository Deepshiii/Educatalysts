...
if(isset($_POST['save']))
{
    ...
    elseif($_FILES['myfile']['size'] > 1000000000000000)
    {
        ...
        {
            $sql = "INSERT INTO jeefiles (filename,size,downloads)
            VALUES('$filename','$size',0)
            ";

            if(mysqli_query($conn,$sql))
            {
                header('location:jeedownloaduser.php');
            }
            else{
...
$sql="SELECT * FROM jeefiles WHERE id=$id";
...
if(file_exists($filepath))
{
    ...
    $updatQuery = "UPDATE jeefiles SET downloads=$newCount WHERE id=$id";
