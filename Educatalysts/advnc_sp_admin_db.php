...
$sql = "SELECT * FROM jeeadvncsamplepaper";
...
if(isset($_POST['save']))
{
    ...
    elseif($_FILES['myfile']['size'] > 10000000)
    {
        ...
        {
            $sql = "INSERT INTO jeeadvncsamplepaper (filename,size,downloads)
            VALUES('$filename','$size',0)
            ";

            if(mysqli_query($conn,$sql))
            {
                header('location:advnc_sp_admin_dwnld.php');
            }
            else{
...
$sql="SELECT * FROM jeeadvncsamplepaper WHERE id=$id";
...
if(file_exists($filepath))
{
    ...
    $updatQuery = "UPDATE jeeadvncsamplepaper SET downloads=$newCount WHERE id=$id";
