...
if(isset($_POST['save']))
{
    ...
    elseif($_FILES['myfile']['size'] > 10000000)
    {
        ...
        {
            $sql = "INSERT INTO programnotes (filename,size,downloads)
            VALUES('$filename','$size',0)
            ";

            if(mysqli_query($conn,$sql))
            {
                header('location:pg_notes_admin_dwnld.php');
            }
            else{
...
$sql="SELECT * FROM programnotes WHERE id=$id";
...
if(file_exists($filepath))
{
    ...
    $updatQuery = "UPDATE programnotes SET downloads=$newCount WHERE id=$id";
